...
def get_players(parsed_data,roundnum, tick, side):
    ...
    # print(f'roundNum:{roundnum-1}')
    alive=0
    teamEqVal=0
    # print(f'get_player prints tick:{tick}')
    if side=="ctSide":
        frameSide="ct"
        killSide="CT"
    else:
    ...
    for frame in round_data["frames"]:
        # print(f'frame[tick]: {frame["tick"]} and tick {tick}')
        if frame["tick"] <= tick:
            # last_tick=frame["tick"]  #checking for if they actually match
            teamEqVal=frame[frameSide]["teamEqVal"]
            for player in frame[frameSide]["players"]:
                # print(f'get_player prints if player in frame: {tick}')
                if player["steamID"] in team_stats:
                    ...
                    # print(f'get_player prints player stat: {team_stats[player["steamID"]]}')
...
def getTotal(demo_data):
    ...
    for roundNum, round in rounds_stats.items():
        ...
        for ID, player in round["t"].items():
            ...
            # rounds_stats[roundNum]["t_team"]["living_players"][ID]["total_damage"]=totalDamage[ID]
            avgDamage[ID]=totalDamage[ID]/(roundNum+1)
...
def getPrompt(demo_data, roundNum, total):
    tick_diff=demo_data["gameRounds"][roundNum]["kills"][1]['tick']-demo_data["gameRounds"][roundNum]["kills"][0]['tick']
    sec_diff=demo_data["gameRounds"][roundNum]["kills"][1]['seconds']-demo_data["gameRounds"][roundNum]["kills"][0]['seconds']
    tick_rate=tick_diff/sec_diff
    tick_values = [second * tick_rate for second in range(3, 16)]
    statHolder=[]
    for kill in demo_data["gameRounds"][roundNum]["kills"]:
        # print('in-prompt')
        random_tick = random.choice(tick_values)
        tlive, tdead, textra=get_players(demo_data, roundNum, kill['tick']-random_tick, "tSide")
        ctlive, ctdead, textra=get_players(demo_data, roundNum, kill['tick']-random_tick, "ctSide")
        for ID, player in tlive.items():
            ...
            if ID==kill['attackerSteamID']:
                player["will_die"]=0
                statHolder.append(player)
            elif ID==kill['victimSteamID']:
                player["will_die"]=1
                statHolder.append(player)
            else:
                statHolder.append(player)
...
# folder_name='Apr'
# directory = f'/mnt/data06/asalinas/csgo/data/2022/{folder_name}'
all_data = []
...
for month in tqdm(months, desc = "Processing month"):
    ### Replace with the right directory path ###
    directory = 'Your Directory'
    # directory = f'/mnt/data06/asalinas/csgo/data/2022/{month}'
        ...
        if filename.endswith('.dem'):
            ...
            try:
                ...
                if isNotNone(parsed_data):
                    ...
                    all_data.append({"error": "None", "original_demo": f"{month}/{filename}"})
                ...
                if "panic" in error_message:
                    error_message = {"error": "panic", "original_demo": f"{month}/{filename}"}
                else:
                    error_message = {"error": "error set", "original_demo": f"{month}/{filename}"}
                all_data.append(error_message)
        clean_up_json_files()

with open('true_label_death_data.json', 'w') as json_file:
